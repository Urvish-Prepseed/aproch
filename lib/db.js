...
export const APROCH_NEWS_COLLECTION = "aprochNews";
...
/** Read/write `aprochNews` without a Mongoose model (schema lives in riverside only). */
export async function getAprochNewsCollection() {
  const conn = await connectToDb();
  return conn.db.collection(APROCH_NEWS_COLLECTION);
}
